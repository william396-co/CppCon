...
class fixed
{
    ...
    {};
    constexpr inline fixed( BaseType val, raw_construct_tag ) noexcept
        : m_value( val ) {}
...
public:
    ...
    constexpr inline explicit fixed( T val ) noexcept
        : m_value( static_cast<BaseType>( ( EnableRounding ) ? ( val >= 0.0 ) ? ( val * FRACTION_MULT + T { 0.5 } ) : ( val * FRACTION_MULT - T { 0.5 } )
                                                             : ( val * FRACTION_MULT ) ) )
    {
    }
    ...
    static constexpr inline fixed from_fixed_point( T value ) noexcept
    {
        ...
        return ( EnableRounding ) ? fixed( static_cast<BaseType>(
                                               value / ( T( 1 ) << ( NumFractionBits - FractionBits ) ) + ( value / ( T( 1 ) << ( NumFractionBits - FractionBits - 1 ) ) % 2 ) ),
                   raw_construct_tag {} )
